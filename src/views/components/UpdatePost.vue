<template>
  <div>
    ...
    <label v-if="reasonPost != ''" style="color: red; font-size: 16px">* Lý do: {{reasonPost}}</label>
      ...
      <div style="width: 50%">
        <el-card>
          ...
          <div style="display: flex; flex: 1; align-items: center; margin-top: 15px">
            ...
            <div>
              <el-input size="medium" v-model="post.colorInside"></el-input>
            </div>
          </div>
          <div style="display: flex; flex: 1; align-items: center; margin-top: 15px">
            <div style="width: 100px; text-align: left">
            ...
            <div>
              <el-input size="medium" v-model="post.colorOutside"></el-input>
            </div>
          </div>
          <div style="display: flex; flex: 1; align-items: center; margin-top: 15px">
            <div style="width: 100px; text-align: left">
          ...
          <div style="display: flex; flex: 1; align-items: center; margin-top: 10px">
            ...
            <div>
              <el-select placeholder="Hộp số" v-model="post.gearType">
                <el-option value="AT" label="Số tay"></el-option>
                <el-option value="MT" label="Số tự động"></el-option>
              </el-select>
            </div>
          </div>
          <div style="display: flex; flex: 1; align-items: center; margin-top: 10px">
    ...
    <div>
        ...
        <div>
          <div style="display: flex">
            <div style="display: flex; align-items: center">
              ...
              <el-input v-model="post.contact.fullName"></el-input>
            </div>
            <div style="display: flex; align-items: center; margin-left: 10px">
              <label style="width: 110px" >Điện thoại:</label>
              <el-input v-model="post.contact.phoneNumber"></el-input>
            </div>
            <div style="display: flex; align-items: center; margin-left: 10px">
              <label style="width: 150px">Điện thoại khác:</label>
              <el-input v-model="post.contact.anotherPhoneNumber"></el-input>
            </div>
          </div>
          <div style="display: flex; align-items: center;margin-top: 10px">
            <div style="display: flex; align-items: center">
              <label style="width: 60px; text-align: left" >Địa chỉ:</label>
              <el-input style="width: 400px" v-model="post.contact.address" size="mini"></el-input>
            </div>
      ...
      <el-card>
        <div style="text-align: left; margin-top: -15px">
          ...
          <div v-if="imageList.length > 0" style="display: flex">
    ...
    <div style="margin-top: 15px">
      <el-button type="success" style="margin-left: 45%" @click="savePost">Lưu</el-button>
    </div>
  </div>
</template>
...
export default {
    ...
    methods:{
        selectedImages(payload){
            this.imageList = this.imageList.concat(payload);
        ...
        async savePost(){
            ...
            if (CheckEmpty.checkInput(this.post.version))
                return this.alertError("Bạn chưa nhập phiên bản");
            if (this.post.yearOfManufacture == null)
                return this.alertError("Bạn chưa chọn năm sản xuất");
            ...
            if (this.post.type == "NEW") this.post.numberOfKmMoved = 0;
            this.post.images = this.imageList;
            this.post.userId = app.user.id;
            var response = await api.callAPI("POST","/api/post/update",this.post);
            if (response.data){
    ...
    beforeMount(){
        ...
        this.post.yearOfManufacture = null;
        this.imageList = this.post.images;
        this.provinces = stores.provinces;
        this.reasonPost = this.reason;
        console.log(this.reasonPost);
    }
    }
</script>
